...
Právní dokumenty (ToS + Privacy) jsou **mimo tyto JSONy** ve složce `/legal/` — viz vlastní sekci níže.
...
8. **Právní texty (ToS + Privacy) překládat netřeba.** Jsou univerzální — pro češtinu se použije česká verze z `/legal/*.md`, pro všechny ostatní jazyky anglická.

## Měna a částky pro každou zemi

Každý jazyk si určuje **vlastní měnu, symbol a pozici symbolu**. Pole v JSONu:

```json
"payment": {
  "currency": "EUR",              // ISO 4217 kód
  "symbol":   "€",                // grafický symbol, který se zobrazuje
  "symbolBeforeAmount": false,    // true = "$5", false = "5 €"
  "defaultAmounts": [1, 5, 10]    // tři přednastavená tlačítka
}
```

A v sekci `ui` totéž, duplicitně (používá se jako rychlá cache při renderu):

```json
"cur": "EUR",   "sym": "€",   "pre": false,   "a": [1, 5, 10]
```

Symbol před/za částkou se řídí zvyklostmi dané země:
- **před** (`pre: true`): USD, GBP, CAD, AUD — „$5", „£10"
- **za** (`pre: false`): EUR (DE/FR/SK), CZK, PLN — „5 €", „100 Kč"

### Částky ve smysluplných hodnotách

Neberte 25/100/500 z CZ doslova a neukládejte 25 CZK jako 25 USD. **Ať na tlačítku sedí odhadem stejná reálná hodnota:**

| Země | classic | middle | hero |
|------|---------|--------|------|
| CZ   | 25 Kč   | 100 Kč | 500 Kč |
| US   | $1      | $5     | $10  |
| SK/DE| 1 €     | 5 €    | 10 € |
| PL   | 5 zł    | 20 zł  | 50 zł  (odhad)

### Jazyky, které nepoužívají arabské číslice

Některé jazyky mají **vlastní systém číslic**. Pole `a: [1, 5, 10]` v JSONu musí vždy zůstat **arabskými číslicemi** (`0-9`), protože s nimi počítá JavaScript. Ale **grafické zobrazení** na stránce může být v rodných číslicích — to si pak vyřeš přes CSS `font-family`, nebo doplnit překladovou vrstvu v renderu. Pro následující jazyky buď obezřetný:

- **Arabština** (`ar`): domorodě ٠١٢٣٤٥٦٧٨٩ (východoarabské), ale většina moderních webů používá 0-9. Pokud chceš autenticky: text zůstane „1 دولار" a JS musí převést displayed číslici přes `Intl.NumberFormat('ar-EG-u-nu-arab')`.
- **Hindština** (`hi`): domorodě ०१२३४५६७८९ (devanagari). Stejná situace.
- **Perština** (`fa`), **thajština** (`th`), **bengálština** (`bn`), **barmština** (`my`) — každý má svůj systém.
- **Čínština** (`zh`), **japonština** (`ja`), **korejština** (`ko`): používají arabské číslice 0-9 v moderním kontextu. OK.

Až budeš tyto jazyky přidávat, ozvi se a upravíme rendering.
...
- **Tlačítka otevírající právní dokumenty**: `tmt` (Terms/Podmínky), `pmt` (Privacy/Zásady) — ale **obsah dokumentů samotných není v těchto JSONech**, viz sekci níže.
- Stavové hlášky u právních dokumentů: `legal_loading`, `legal_error`.
- Všechny FAQ otázky a odpovědi (`faq[]`).

## Právní dokumenty (Terms + Privacy) — UNIVERZÁLNÍ

Podmínky užití a Zásady ochrany osobních údajů leží v samostatné složce `/legal/`
jako čisté markdown soubory:

```
/legal/
  terms.md      ← oba jazyky (EN + CS) v jednom souboru
  privacy.md    ← oba jazyky (EN + CS) v jednom souboru
```

Každý soubor obsahuje obě jazykové verze oddělené nadpisy `# 🇬🇧 ENGLISH VERSION`
a `# 🇨🇿 ČESKÁ VERZE`. **Stejné texty platí pro všechny země** — anglická verze
pokrývá všechno mimo češtiny, česká verze jen pro CS. Není potřeba překládat do
němčiny, francouzštiny atd.

**Jak to funguje:** Když uživatel klikne na tlačítko "Podmínky/Terms" nebo
"Soukromí/Privacy", JS lazy-loadne daný MD soubor, vytáhne z něj sekci
odpovídajícího jazyka (CS pro české UI, jinak EN), provede mini markdown →
HTML převod a zobrazí v modálu.

**Podporovaná markdown syntaxe:** `#`/`##`/`###` nadpisy, `**tučné**`,
odrážky `- …`, tabulky `| … |`, vodorovné čáry `---`, odstavce, auto-linky
URL a e-mailů. Blockquotes `> …` se zahazují (v originále jsou to jen
poznámky pro autora). Pokud byste chtěli syntax rozšířit, úpravy jsou v
`js/app.js` ve funkci `mdToHtml`.

**Úpravy textu dokumentů:** stačí editovat přímo `legal/terms.md` a
`legal/privacy.md` — není potřeba sahat do JS ani do JSON. Zachovejte strukturu
s oběma jazykovými hlavičkami.
